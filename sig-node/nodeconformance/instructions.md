...
```
docker run --env KUBECONFIG=/root/.kube/config --env RESULTS_DIR=/tmp/results --env E2E_PARALLEL="--procs=4" -v /root/.kube/config:/root/.kube/config -v /tmp/results:/tmp/results --network host -it kubeedge/nodeconformance:v1.23.1
```
...
| E2E_PARALLEL          | p                                   | If set, its value is passed to ginkgo to enable parallel test execution. The recommended setting is `--procs=4`.                 |
...
Prepare a PR to [https://github.com/kubeedge/community](https://github.com/kubeedge/community) to upload the report files to the directory `sig-node/nodeconformance`. In the descriptions below, `X.Y` refers to the KubeEdge major and minor version, and `$dir` is a short subdirectory name to hold the results for your product.
